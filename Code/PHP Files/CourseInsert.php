        ...
        <select class="it" name="cin" id="cin" class="it" required>
            ...
            <?php
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    $cin = $row['CIN'];
                    echo "<option class='it' value='$cin'>$cin</option>";
                }
            ?>
        </select>
        <br><br>
        <label for="degree"><h3 class="warn">Select a Degree Program:</h3></label>
        <br>
        <input type="radio" id="bsc" name="degree" value="B.Sc CS" required>
        <label for="bsc" class="radio">B.Sc CS</label>
        <br><br><br>
        <input type="radio" id="msc" name="degree" value="M.Sc CS" required>
        <label for="msc" class="radio">M.Sc CS</label>
        <br><br>
        <center><button class="submit" type="submit">Submit</button></center>
    </form>
    <div id="validation-message" style="display: none; color: red;">
      Please select exactly one degree program.
    </div>
</div>
    <script>
...
{	
	...
	if ($result->num_rows > 0) 
	{
	$check_query= "SELECT * from courseinfo where CIN= '$cin'";
	$result = $conn->query($check_query);
	if ($result->num_rows > 0) 
	{
    // CIN already exists, so display an error message
    echo "<script>alert('Error: CIN already exists in the Course Information Table');</script>";
	}
	else
	{
		$insert_query = "INSERT INTO courseinfo (CIN,course) VALUES ('$cin','$course')";
